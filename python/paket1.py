...
def apply(proc, hidden_cov, posisi, dimulai_dari='1'):
    ...
    dimulai_dari : '1' | '2' | '3' | '4' — halaman pertama yang terlihat
    """
    align = proc._get_h_align(posisi)
    top   = proc._is_top(posisi)
    try:
        start_num = int(dimulai_dari)
    ...
    for i, section in enumerate(proc.doc.sections):
        ...
        if i == 0 and hidden_cov == 'Ya':
            # cover_start = start_num - 1 → cover 1 tersembunyi, halaman 2+ mulai start_num
            ...
            proc.set_page_number_format(section, 'decimal', start_num if i == 0 else None)
