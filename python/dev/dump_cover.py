"""Dump hanya area cover + awal zona romawi dari dua file docx."""
import zipfile, sys
...
def dump(path, label, max_el=120):
    with zipfile.ZipFile(path) as z:
        xml = z.read("word/document.xml")
    root = etree.fromstring(xml)
    body = root.find('.//{%s}body' % W)
    children = list(body)
    print('=== %s  (total body els: %d) ===' % (label, len(children)))
    for i, el in enumerate(children):
        if i >= max_el:
            print('  ... (truncated at %d)' % max_el)
            break
        tag = el.tag.split('}')[-1]
        if tag != 'p':
            print('[%03d] [%s]' % (i, tag.upper()))
            continue
