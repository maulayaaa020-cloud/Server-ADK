...
for i in range(max(0, rsp_idx-5), min(len(body_els), rsp_idx+20)):
    el  = body_els[i]
    ...
    mk2  = ' <-- RSP' if i == rsp_idx else ''
    mk2 += ' <-- BAB' if el in bab_p_list else ''
    print(f"  [{i:3d}] {tag:<5}  {mark}  {txt(el)!r}{mk2}")
